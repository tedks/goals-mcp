...
test('compiled distribution: discovery, human guidance, validation, HTTP, workflow errors and shutdown', { timeout: 180000 }, async () => {
  ...
  try {
    ...
    const guideDocument = readFileSync(resolve(__dirname, '../docs/AGENTS.md'), 'utf8');
    assert.equal(guideDocument, '# Collaborating with a person through Goals\n\n' + client.getInstructions().replaceAll('\n', '\n\n') + '\n');
    const guideResource = await client.readResource({ uri: 'goals://guide' });
    assert.equal(guideResource.contents[0].text, client.getInstructions());
